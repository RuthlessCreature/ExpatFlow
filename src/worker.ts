...
function requireAuth(request: Request, env: Env): Response | null {
  if (!env.BRIDGE_TOKEN) {
    return json({ error: "BRIDGE_TOKEN is not configured" }, 503);
  }
...
async function startLiveSession(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    browser.disconnect();
    return json({ ok: true, sessionId, liveViewUrl, url: page.url(), title: await page.title(), keepAliveMs: KEEP_ALIVE_MS, liveViewExpiresMs: LIVE_VIEW_EXPIRES_MS, sessionMode: "puppeteer-disconnect", next: "Open liveViewUrl. The remote Browser Run session remains alive after the Worker disconnects." });
  } catch (error) {
...
async function inspectLiveSession(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const liveViewUrl = await createLiveView(page);
    return json({ ok: true, sessionId, url: page.url(), title: await page.title(), readyState: await page.evaluate(() => document.readyState), cookieCount: cookies.length, cookieDomains: [...new Set(cookies.map((cookie) => cookie.domain))].sort(), liveViewUrl, liveViewExpiresMs: LIVE_VIEW_EXPIRES_MS, sessionMode: "puppeteer-reconnect" });
  } finally {
    browser.disconnect();
  }
}
...
async function gotoInSession(request: Request, env: Env): Promise<Response> {
  ...
  try {
    await page.goto(targetUrl, { waitUntil: "domcontentloaded", timeout: 45_000 });
    return json({ ok: true, sessionId, url: page.url(), title: await page.title(), sessionMode: "puppeteer-reconnect" });
  } finally {
    browser.disconnect();
  }
}
...
async function pingSession(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return json({ ok: true, sessionId, readyState, href, ts: Date.now(), sessionMode: "puppeteer-reconnect" });
  } finally {
    browser.disconnect();
  }
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (!pathname.startsWith("/api/live/")) return legacyWorker.fetch(request, env, ctx);
    const authError = requireAuth(request, env);
    if (authError) return authError;
    try {
      if (pathname === "/api/live/start" && request.method === "POST") return await startLiveSession(request, env);
      if (pathname === "/api/live/inspect" && request.method === "GET") return await inspectLiveSession(request, env);
      if (pathname === "/api/live/goto" && request.method === "POST") return await gotoInSession(request, env);
      if (pathname === "/api/live/ping" && request.method === "POST") return await pingSession(request, env);
      return json({ error: "Not Found" }, 404);
    } catch (error) {
      return json({ error: errorMessage(error), hint: "If the browser was closed or idle beyond keep_alive, start a new session." }, 500);
    }
  },
};
